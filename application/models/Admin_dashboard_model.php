...
class Admin_dashboard_model extends CI_Model {
    ...
    public function get_page_info_model() {
        // $this->db->select("count(id) as new_faxes_count")->from("efax_info");
        // $this->db->where("create_datetime < ", "now()", false);
        // $this->db->where("create_datetime >= ", "date_sub(now(),INTERVAL 1 WEEK)", false);
        // $new_faxes_count = $this->db->get()->result()[0]->new_faxes_count;


        // $this->db->select("count(id) as new_tasks_count")->from("clinic_physician_tasks");
        // $this->db->where("create_datetime < ", "now()", false);
        // $this->db->where("create_datetime >= ", "date_sub(now(),INTERVAL 1 WEEK)", false);
        // $new_tasks_count = $this->db->get()->result()[0]->new_tasks_count;

        // $this->db->select("count(id) as completed_tasks_count")->from("clinic_physician_tasks");
        // $this->db->where("active = 0");
        // $this->db->where("create_datetime < ", "now()", false);
        // $this->db->where("create_datetime >= ", "date_sub(now(),INTERVAL 1 WEEK)", false);
        // $completed_tasks_count = $this->db->get()->result()[0]->completed_tasks_count;
        ...
        return array(
            ...
            "api_calls" => $api_calls,
            // "new_faxes_count" => $new_faxes_count,
            // "new_tasks_count" => $new_tasks_count,
            // "completed_tasks_count" => $completed_tasks_count
        );
    }

    public function ssp_clinic_statistics_model() {
        $table = "view_tracker_dash_clinics";
        $primaryKey = "id";
        $columns = array(
            array('db' => 'clinic_institution_name', 'dt' => 0),
            array('db' => 'signup_date', 'dt' => 1),
            array('db' => 'physicians_count', 'dt' => 2, 'formatter' => function( $d, $row ) {
                    return ($d!=null)?$d:0;
                }),
            array('db' => 'new_referrals', 'dt' => 3, 'formatter' => function( $d, $row ) {
                    return ($d!=null)?$d:0;
                }),
            array('db' => 'accepted_referrals', 'dt' => 4, 'formatter' => function( $d, $row ) {
                    return ($d!=null)?$d:0;
                }),
            array('db' => 'scheduled_referrals', 'dt' => 5, 'formatter' => function( $d, $row ) {
                    return ($d!=null)?$d:0;
                }),
            array('db' => 'visits_confirmed', 'dt' => 6, 'formatter' => function( $d, $row ) {
                    return ($d!=null)?$d:0;
                }),
            array('db' => 'faxes_sent', 'dt' => 7, 'formatter' => function( $d, $row ) {
                    return ($d!=null)?$d:0;
                }),
            array('db' => 'demographic_api', 'dt' => 8, 'formatter' => function( $d, $row ) {
                    return ($d!=null)?$d:0;
                }),
            array('db' => 'clinical_api', 'dt' => 9, 'formatter' => function( $d, $row ) {
                    return ($d!=null)?$d:0;
                }),
            array('db' => 'physician_api', 'dt' => 10, 'formatter' => function( $d, $row ) {
                    return ($d!=null)?$d:0;
                }),
            array('db' => 'data_points_captured', 'dt' => 11, 'formatter' => function( $d, $row ) {
                    return ($d!=null)?$d:0;
                })
        );
