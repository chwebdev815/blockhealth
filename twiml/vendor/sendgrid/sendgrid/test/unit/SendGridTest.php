...
class SendGridTest extends BaseTestClass {
    ...
    public function testCanConnectToSendGridApi() {
        ...
        $this->assertEquals(
                $sg->client->getHost(), 'https://api.sendgrid.com', '/v3'
        );
        $this->assertEquals(
                $sg->client->getHeaders(), $headers
        );
        ...
        $sg4 = new \SendGrid(
                self::$apiKey, ['curl' => [CURLOPT_PROXY => '127.0.0.1:8000']]
        );
        $this->assertEquals(
                $sg4->client->getCurlOptions(), [10004 => '127.0.0.1:8000']
        );

        $subuser = 'abcxyz@this.is.a.test.subuser';
        $headers[] = 'On-Behalf-Of: ' . $subuser;
        $sg5 = new \SendGrid(
                self::$apiKey, ['impersonateSubuser' => $subuser]
        );
        $this->assertSame(
                $headers, $sg5->client->getHeaders()
        );
    }
